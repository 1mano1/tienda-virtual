...
function createPurchase(req, res) {
  ...
  // Sanitizar el nombre del cliente para prevenir XSS
  const sanitizedName = xss(customerName);
  ...
  return res.status(201).json({ id: randomUUID(), customerName: sanitizedName, items, total, status: 'created' });
}
